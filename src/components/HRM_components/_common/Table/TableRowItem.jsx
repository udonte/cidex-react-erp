...
const TableRowItem = ({ children, onClick, dotOptions, id }) => {
  ...
  return (
    <tr
      onClick={onClick}
      className="pl-6 capitalize items-center text-left bg-white h-[60px] text-lg text-gurugeeks-text font-medium border-y-2 hover:bg-orange-50"
    >
      {children}
      {dotOptions && (
        <td
            ...
            >
              {dotOptions?.map((item, index) => (
                <div
                  ...
                  key={index}
                  className=" cursor-pointer px-6 py-3 gap-x-3 w-full min-w-max text-sm text-gray-500  flex items-center shrink-0 hover:bg-gurugeeks-dark-100"
                >

...
const FilterTab = ({
  ...
  const handleFiltersSelect = (filterOption, selectedFilter) => {
    selectedFilters?.map((item) => item.name).includes(filterOption)
      ...
      : setSelectedFilters((prev) => [
          ...prev,
          { name: filterOption, option: selectedFilter.name },
        ]);
    console.log(filterOption, selectedFilter);
  };
  ...
  return (
    <>
      <div className="flex justify-between items-end py-2 px-6 z-10">
        {filter ? (
                  ...
                  {showFiltersOptionsindex === index &&
                    ...
                    option.list.map((item, index) => (
                      <div key={index}>
                        <div
                          onClick={() => {
                            handleFiltersSelect(option.name, item);
                          }}
                          className="h-10 px-2 capitalize z-50  flex items-center bg-white border hover:bg-slate-50 border-gray-300 shadow-lg"
                        >
                          {item.name}
                        </div>
                      </div>
      ...
      <div className="px-6 my-2 flex-wrap flex items-center gap-x-2">
        {selectedFilters?.map((item) => (
          <div className=" bg-green-200 h-8 w-fit rounded flex items-center justify-center px-2 my-2">
            <p>
              {item.name}:{" "}
              <span className="font-bold pr-2">"{item.option}"</span>
            </p>
            <RiCloseFill
              onClick={() => {
